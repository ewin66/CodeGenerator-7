...
namespace CodeGenerator.BusinessService.Base_SysManage
{
    ...
    {
        //使用的数据库连接
        public Base_UserService() : base("BaseDb") {
        ...
        public List<Base_UserModel> GetDataList(string condition, string keyword, Pagination pagination)
        {
            ...
            {
                item.RoleIdList = Base_UserService.GetUserRoleIds(item.UserId);
                item.RoleNameList = item.RoleIdList.Select(x => Base_SysRoleService.GetRoleName(x)).ToList();
                item.RoleNames = string.Join(",", item.RoleNameList);
                ...
                {
                    if (item.RoleNames.Contains(aValue.ToString()))
                        type += (int)aValue;
                }
                item.RoleType = (EnumType.RoleType)type;
            }
            return list;
        }
    ...
    public class Base_UserModel : Base_User
    {
        ...
        public EnumType.RoleType RoleType { get; set; }
        //public List<string> RoleIdList { get => Base_UserService.GetUserRoleIds(UserId); }
        //public List<string> RoleNameList { get => RoleIdList.Select(x => Base_SysRoleService.GetRoleName(x)).ToList(); }
